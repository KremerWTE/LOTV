...
- [ ] **JotForm webhook (`POST /api/v1/webhooks/jotform`, `Program.cs`) has real data-integrity bugs** — the `knownLabels` list used to split JotForm's "pretty" submission string is now out of sync with the live form after this session's field cleanup (deleted "How did you hear about us?" and "Quarterly Grief Support Interest" no longer match their replacement dropdowns' different label text). Independently of this session, **"Children for Bracelet" — a required field on every submission — was never in `knownLabels` at all**, so its answer silently glues onto the end of the preceding field's value and `Family.ChildrenInitials` comes through `null` on every real submission to date. Not yet fixed in code; needs a `knownLabels` update plus verification against a live test submission before the next real intake.
