...
- [x] Choose hosting — **decided 2026-08-31**: Azure App Service. `deploy-staging.yml`/`deploy-production.yml` now log in via `azure/login` and deploy each image with `azure/webapps-deploy`; **still needed**: provision the App Service resources and add the GitHub secrets (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AZURE_WEBAPP_API_NAME`, `AZURE_WEBAPP_WEB_NAME`, `AZURE_WEBAPP_API_NAME_PROD`, `AZURE_WEBAPP_WEB_NAME_PROD`) — needs Azure account access I don't have
...
- [ ] **Hosting platform still undecided** — user confirmed a decision was made but didn't specify which platform (Azure/Railway/Fly.io/AWS/other) or share account details when asked twice; the actual `deploy-staging.yml`/`deploy-production.yml` "deploy to X" step remains a commented-out placeholder. Needs the platform name before this can be finished.
...
- [x] **Verified webhook field coverage against the ministry's real, currently-active operational spreadsheet** (`Prayer Care Package Request Database.xlsx`, `2026` sheet — 1,276 real rows) instead of assuming the JotForm schema alone was ground truth. Confirmed the `SM (2026)` sheet (their live Stephen's Ministry tracking) has exact 1:1 field parity with the already-built `FollowUpTracker`/`FollowUpMilestone` model, and derived the exact milestone offset formula from real due-date deltas in that sheet: 3 Weeks = DateOfLoss + 21 days, 3/6/11 Months = DateOfLoss.AddMonths(3/6/11).
...
- [ ] **Confirm with ministry staff whether the live Kanban's near-empty pipeline matches actual current operations** — purely a question for the ministry, nothing to build; still open.
