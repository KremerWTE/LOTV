...
**Last Updated**: 2026-07-27 (username auth + real data import — see sessions/2026-07-27-username-auth-and-real-data-import.md) — migrated staff sign-in from email to username (firstname.lastname) with forgot/reset-password flow; cross-linked + cleaned up Kanban/Queue/My-Work-Queue; imported the ministry's real historical spreadsheet (1,046 cases, 425 Mother's Day mailing entries, 47 bereavement follow-up trackers) into a real SQL Server production database at 10.100.1.87 — Phase 6 database hosting decision now made
**Previous Update**: 2026-07-09 (local QA pass — see sessions/2026-07-09-qa-bugfix-and-nav-restructure.md) — fixed 2 fatal duplicate-route crashes (`/admin/impact`, `/admin/migrations`), stale SignalR port causing infinite dashboard spinner, JWT missing name claims (GUID shown instead of user name), ~57 un-decoded HTML entity codes across 22 files; sidebar reorganized into Overview/Donations/Programs/Retreats/Reports/Admin to match `docs/mockup-admin-dashboard.html`; local Duda + GiveButter webhook wiring validated via cloudflared tunnel + dotnet user-secrets
...
- [ ] **EF migration history needs reconciling for SQL Server** — migrations were scaffolded against SQLite and bake literal SQLite column types into their `Up()` methods; `dotnet ef database update`/`Database.Migrate()` doesn't produce correct SQL Server DDL from them. Worked around via `EnsureCreated()` for full-schema init and a hand-written T-SQL script for the one incremental table addition needed this session — not a long-term fix
- [ ] **No admin UI for the bereavement follow-up tracker yet** — `FollowUpTracker`/`FollowUpMilestone` data is imported and live but has nothing to view/manage it in the app, unlike Historical Cases and Mother's Day Mailing
- [ ] **Rotate the `sa` SQL Server credential** used to set up the `LOTV` database this session; create a dedicated least-privilege login for the app to use going forward
- [ ] Confirm with ministry staff whether the live Kanban board's near-empty pipeline (1 open case; ~1,040 of 1,046 imported cases already show "Fulfilled" per the spreadsheet's own tracking notes) matches actual current operations before relying on it
