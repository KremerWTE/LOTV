...
- [ ] **Not verified: individual page behavior beyond the home/login route.** This app has ~100+ admin pages (per the 2026-07-09 nav-reorg session: 101 pre-existing links). The conversion is structurally sound (no sync JS interop was found anywhere in the codebase — confirmed via repo-wide grep before starting, which was the main WASM→Server risk), but no page beyond `/` and `/login` was actually exercised against a real login + real data. Needs a real walkthrough (log in, hit Kanban, Follow-Up Trackers, Historical, reports, drag-and-drop, CSV/PDF export, the Leaflet map pages, SignalR-driven live dashboard updates) before trusting this in front of the ministry.
