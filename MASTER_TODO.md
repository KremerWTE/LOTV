...
- [x] Localization / i18n — `LocalizationService` (en/es), `LanguageSwitcher.razor` component (persisted to localStorage), wired into `PublicLayout` header + `Home.razor`; remaining public pages (Apply, Give, VolunteerSignup, Help, Transparency, Events) ready to migrate to the same `@inject LocalizationService Loc` + `@Loc["key"]` pattern as additional strings are added to the dictionary
