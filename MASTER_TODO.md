...
- [x] **Real browser walkthrough done post-conversion — found and fixed a genuine, significant bug.** Login → Dashboard → Kanban → Case Detail → Case Analytics (heat map + the Leaflet "Cases Map", the app's highest-risk JS interop surface) → Follow-Up Trackers → Historical Cases → Families by State, all with real seeded data.
  ...
  - **Kanban drag-and-drop: inconclusive, not confirmed either way.** The browser-automation tool's synthetic mouse drag didn't visibly move a card, but logged no console errors — more likely an HTML5-native-drag-and-drop automation limitation than a real regression, but a human should actually try dragging a card before trusting it.
  - **Still not walked**: CSV/PDF export (the `window.downloadCsv` JS interop function specifically), Mother's Day Mailing, Bulk Update, and the remaining ~90 admin pages beyond what was checked above.
