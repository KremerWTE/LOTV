...
**Last Updated**: 2026-09-14 (pateep_dev_branch merged origin/kremer-dev — see remote commits below)
...
- [x] Write `deploy-to-iis.yml` — completed 2026-09-09: 6-job pipeline (build+test → run-migrations → deploy-web → deploy-api → tag-and-release → send-notification); PointShopMall dual-project pattern + Boneforte secret injection + LOTV EF migrations step; correct config key paths verified (Stripe:*, Twilio:*, Push:Vapid*)
...
> **Superseded 2026-09-10**: every open JotForm item above (this section and `docs/jotform-remaining-fixes.md`) is now moot — JotForm was removed from the app entirely per direct client instruction. See the new session section immediately below and R-26 in `docs/LOTV-PM-Plan.md`. Left the historical entries above unedited as an accurate record of what was investigated and why; none of it needs action anymore.

### Stripe/JotForm Removal, GiveButter Donations, Duda Intake Form, Admin Nav Consolidation, Board/Director Roles, Live Smoke Test — 2026-09-10 session
*Full details in sessions/2026-09-10-stripe-jotform-removal-givebutter-duda-intake-roles.md and `docs/LOTV-PM-Plan.md` risk register R-26 through R-31*

- [x] **Stripe removed from the app entirely** — direct client instruction ("we don't need to have Stripe or JotForm", confirmed as full-app-scope removal, not just the payment endpoints). Deleted: `/api/v1/payments/intent`, `/api/v1/payments/webhook`, Stripe customer creation in `/give`, billing-portal endpoint, `SyncStripeRecurringAsync` and its 3 call sites (pause/cancel/resume now just update local DB status directly), Payment Reconciliation report page, the Stripe-only webhook-replay endpoint, `donorsWithStripeCustomer` diagnostic, `IPaymentService` (unused/unimplemented interface), the `Stripe.net` package reference, and both `JotFormWebhookTests`/`PaymentWebhookTests` test files (441 lines). `Donor.StripeCustomerId`/`Donation.StripePaymentIntentId` fields remain in the schema (always null now) — harmless, not yet cleaned up.
- [x] **JotForm webhook handler deleted** (`POST /api/v1/webhooks/jotform`, ~255 lines including `knownLabels`, `Field()`, `SplitName()`, `SplitAddress()`). Per the 2026-09-01 HIPAA-BAA finding, this pipe had never received a single real submission anyway.
- [x] **Built a standalone Duda-embeddable intake form** (`docs/duda-embed/prayer-care-intake.html`) to replace it — mirrors JotForm's real conditional logic ("For Me"/"For Someone Else", pulled live from the old form's own `conditions` API data before removal), posts directly to a new `POST /api/v1/public/apply` handler, no third-party HIPAA-BAA account involved. Bracelet field rebuilt to exactly match JotForm's Configurable List structure (repeatable Order/Initial/Bead Type/Living-or-Deceased rows). Husband's/Wife's Name + Email required, phone optional, matching the ministry's real live intake form at lotvministry.org. Confirmation + staff/team notification emails wired in (see below). **Not yet deployable**: `API_BASE_URL` is still a placeholder and no Azure App Service exists to point it at (see Phase 6 Infrastructure Setup, unchanged blocker).
- [x] **GiveButter embedded everywhere Stripe used to be** — the real JS widget (`widgets.givebutter.com`, account `esQKeyJhOUHIb71A`, same one already live on lotvministry.org/donate) on the intake confirmation screen and a fully rewritten `Give.razor`. Required piercing two levels of the widget's shadow DOM at runtime (`givebutter-widget` host → nested `givebutter-giving-form`, which hardcodes `max-width:440px` on itself) to make it fill its container width instead of staying pinned to a fixed max-width — documented as fragile-by-design in code comments since it depends on GiveButter's undocumented internal structure and could break silently on a library update.
- [x] **Admin sidebar consolidated** — 120 total links (57 always-visible) collapsed into 3 tabbed hub pages (`CasesHub.razor` — 11 tabs, `ReportsHub.razor` — role-filtered tabs, `SystemAdminHub.razor` — 10 tabs) per direct client feedback ("there are so many tabs we need to streamline it"), extending the app's pre-existing hub pattern rather than inventing a new one. Each hub embeds the existing routable components as child components (routes still work directly/bookmarked); a "Show merged/legacy links" toggle preserves full backward-compatible reachability of every individual link.
- [x] **`Director` and `Board` roles added** to `UserRole`, plus `Volunteer.Level` (New/Standard/Senior/Lead tenure tier, surfaced in the volunteer self-service portal). Director = near-admin (extended into `ChapterAdmin`/`Staff`/`Volunteer` authorization policies). Board = reports-only, via a dedicated `/board/portal` page and a genuinely separate, aggregate-only `/api/v1/board/*` route group — deliberately not reusing the existing `/api/v1/dashboard/*` group, which despite looking aggregate actually returns raw PII-bearing donor lists to the client. Verified live end-to-end via Playwright for the Board path (real data, zero console errors, zero PII exposure); Director's AdminLayout integration is build-verified but not independently Playwright-walked yet.
- [x] **Found and fixed: `NotificationService` was a stub that only logged.** Its own doc comment said "replace with SendGrid implementation for production" — every "sent" email in the entire app (password resets, broadcasts, portal links, the new intake confirmations) was silently a no-op in any real deployment. Replaced with a real `System.Net.Mail.SmtpClient` implementation, config-driven via a new `Smtp` appsettings section; still falls back to logging when unconfigured so local dev/tests are unaffected. Also widened the new-request team notification from one hardcoded address to `Notifications:IntakeTeamEmails` (comma/semicolon list), wired to the previously-unused `NOTIFICATION_EMAIL_RECIPIENTS` GitHub secret.
- [x] **Found and fixed: `Database:Provider` was unset** in `appsettings.Production.json`/`Staging.json` — a real deploy would have silently fallen through to the untested Postgres branch instead of SQL Server. Added `"Database": {"Provider": "SqlServer"}` to both (not a secret, safe to commit).
- [x] **Found and fixed: neither deploy workflow ever configured the deployed App Service's own runtime settings** — zip-deploy shipped code but the running app would have had no DB connection, no JWT key, no SMTP, no CORS allowlist. Added `az webapp config appsettings set` steps to both `deploy-production.yml`/`deploy-staging.yml`.
- [x] **Ran a real live smoke test against the actual running app** (not just code review or unit tests) — auth, request lifecycle, volunteers/donors, dashboard stats, notes, and all 3 new hub pages plus `/board/portal`, all walked end-to-end with real HTTP calls and a real browser. Caught a process-hygiene lesson along the way: the API instance still running from an earlier Playwright session was serving a 3-day-old build that predated this entire session's work — always kill lingering `dotnet run` processes and rebuild before trusting a "live" test.
- [x] **Found and fixed via that smoke test: every case-activity/note entry showed the staff member's raw GUID instead of their name** (`ActorName`/`AuthorName` were literally `ctx.UserId`, 11 call sites — `IChapterContextService` never exposed a display name at all). Added `IChapterContextService.UserName` from the JWT's given/surname claims. Also fixed `POST /requests/{id}/notes` throwing an unhandled 500 on missing content instead of a clean 400.
- [x] All test data created during live testing cleaned out of the dev DB afterward; 415/415 tests passing after every change in this session.
- [ ] **Still open, same underlying blocker as R-20/R-31**: no Azure App Service exists yet. `API_BASE_URL`/`AllowedOrigins`/`DUDA_SITE_ORIGIN` need real values once the Duda domain and API host are known; `JWT_SIGNING_KEY`/`DB_CONNECTION_STRING` secrets don't exist; SMTP secrets exist but have never been verified as live credentials.
- [ ] `Donor.StripeCustomerId`/`Donation.StripePaymentIntentId` schema remnants — low priority cleanup.
- [ ] Route-walk coverage: today's 3 new hub pages + Board portal spot-checked with zero errors, but the ~285-route gap noted in the 2026-08-31 Extended Route Sweep is otherwise unchanged.
