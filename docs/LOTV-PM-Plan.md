...
**Document Version:** 1.0 | **Date:** 2026-03-02 | **Status:** Active | **Last Updated:** 2026-09-24
...
| Payments | GiveButter embedded widget (Stripe removed 2026-09-10, see R-26) |
...
| R-03 | Stripe integration complexity (webhook reliability) | N/A | N/A | **No longer applicable 2026-09-10**: Stripe removed from the app entirely in favor of GiveButter's embedded widget — see R-26. Left in the register as a record of a risk that resolved via a platform-choice change rather than direct mitigation, same treatment as R-07 |
...
| R-19 | Third-party form-builder tool (JotForm) edits are not reliably atomic — an edit call can report success while silently corrupting unrelated settings | Low | High | Discovered 2026-08-05: a targeted email-settings edit corrupted ~40 unrelated form-level properties in one call. **Root cause confirmed 2026-09-01**: this JotForm account has a signed HIPAA Business Associate Agreement on file (`baaSubmissionID` present at the account level) — JotForm deliberately discards all programmatic writes on BAA accounts to preserve their compliance audit trail, so every "corruption" and every subsequent silent-no-op was this same platform policy, not an intermittent reliability bug. Confirmed via 9 total failed write attempts across this project's life, spanning 3 different endpoints and 5 field types, every one reporting `200 success` while writing nothing. **Not fixable from our side** — every JotForm change from here on goes through the builder UI by hand; the assistant does not have and cannot obtain programmatic write access to this form. Probability downgraded from Medium to Low since this is now a known, stable constraint rather than an unpredictable failure mode. Still open: HIPAA toggle, autoresponder/notification sender names, "How did you hear" wording, pre-checked opt-in checkboxes, Reason dropdown "Other" option, 3-page split, donation-nudge implementation (wording drafted 2026-09-01), and the ministry's own manual revision-history rollback confirmation from the 2026-08-05 incident (added 2026-08-05) |
| R-25 | `Family.DateOfLoss` — the field driving the entire bereavement follow-up tracker's milestone math — was captured correctly but displayed nowhere in the app UI at all | Low | Medium | **Fixed 2026-09-02**: found via a spreadsheet-vs-app field-coverage review (no production DB access — see Assumptions & Constraints); confirmed via a repo-wide grep that zero `.razor` files referenced it before this fix. Added to Case Detail's Family panel, along with the full shipping address (street + apt, not just city/state — staff previously had no way to see the address needed to ship a package without navigating to a separate Family Detail page), Diocese, "How did you hear", and Story. Verified live against several real seeded cases, zero console errors (added 2026-09-02) |
...
| R-26 | Direct client instruction: remove Stripe and JotForm entirely — "everything for donation is through GiveButter," and intake moves to a standalone Duda-embeddable form instead of JotForm | N/A | N/A | **Done 2026-09-10**: All Stripe code removed (payment-intent/webhook/billing-portal endpoints, `SyncStripeRecurringAsync`, `IPaymentService`, `Stripe.net` package reference, Payment Reconciliation report page, `donorsWithStripeCustomer` diagnostic, `PaymentWebhookTests`); JotForm webhook handler and `JotFormWebhookTests` deleted. New standalone intake form built at `docs/duda-embed/prayer-care-intake.html` — mirrors the real JotForm conditional logic ("For Me"/"For Someone Else"), posts directly to `Lotv.Api`'s `/api/v1/public/apply`, no third-party HIPAA-BAA account involved. GiveButter's real JS widget (`widgets.givebutter.com`, account `esQKeyJhOUHIb71A`) embedded on both the intake confirmation screen and the rewritten `Give.razor` — required piercing two levels of the widget's shadow DOM at runtime to make it fill its container width, since it hardcodes `max-width:440px` internally; documented as fragile-by-design in code comments since it depends on GiveButter's undocumented internal structure. This directly supersedes R-18/R-19/R-21's JotForm-specific open items (HIPAA toggle, 3-page split, donation-nudge scoping, etc.) — none of that work is relevant anymore. **Residual gap**: `Donor.StripeCustomerId`/`Donation.StripePaymentIntentId` fields remain in the schema (always null now) — harmless but not yet cleaned up (added 2026-09-10) |
| R-27 | Admin sidebar had grown to 120 total links (57 always-visible) across many admin sessions of incremental feature addition, becoming hard to navigate — direct client feedback: "there are so many tabs we need to streamline it" | Low | Medium | **Fixed 2026-09-10**: extended the app's existing hub-consolidation pattern rather than inventing a new one. Cases, Reports, and System Admin clusters collapsed into tabbed hub pages (`CasesHub.razor` — 11 tabs, `ReportsHub.razor` — role-filtered tabs, `SystemAdminHub.razor` — 10 tabs) that embed the existing routable components as child components (each retains its own `@page` route, so direct links/bookmarks still work); a "Show merged/legacy links" toggle preserves full backward-compatible reachability of every individual link. Client explicitly chose to fold Cases in too, despite it containing high-frequency daily-use tools, after being flagged that this was a real tradeoff (added 2026-09-10) |
| R-28 | Direct client instruction to add role-based view splitting for Director, Board, and volunteer tenure level, beyond the existing HQAdmin/ChapterAdmin/ChapterStaff/Volunteer/Donor roles | Low | High | **Done 2026-09-10**: `Director` (near-admin — same AdminLayout access as staff, extended into `ChapterAdmin`/`Staff`/`Volunteer` policies) and `Board` (reports-only) added to `UserRole`. Board gets a dedicated `/board/portal` page and a genuinely separate `/api/v1/board/*` route group — deliberately NOT reusing the existing `/api/v1/dashboard/*` group, which despite looking aggregate actually returns raw PII-bearing donor lists to the client for browser-side aggregation; the Board endpoints are aggregate-only at the API level so there's no PII exposure path even if the frontend changed. `Volunteer.Level` (New/Standard/Senior/Lead tenure tier, distinct from `VolunteerRole`) added and surfaced in the volunteer self-service portal (onboarding checklist for New, recognition panel for Senior/Lead) per the client's clarified scope — "tiers within the existing Volunteer role," not a new admin-side role. Verified live via Playwright end-to-end for the Board path (real data, zero console errors, zero PII exposure); Director's AdminLayout integration is implemented and build-verified but not independently Playwright-walked (added 2026-09-10) |
| R-29 | `NotificationService` was a stub that only logged every "sent" email — discovered while wiring the new intake form's confirmation/staff-notification emails, meaning production would have looked like it was notifying people while silently notifying no one | Low | High | **Fixed 2026-09-10**: real SMTP implementation (`System.Net.Mail.SmtpClient`, config-driven via a new `Smtp` appsettings section) replaces the stub; still falls back to logging when `Smtp:Host` isn't configured, so local dev/tests are unaffected (415/415 passing). New-request team notification also fixed from a single hardcoded address to `Notifications:IntakeTeamEmails` (comma/semicolon-separated), wired to the previously-unused `NOTIFICATION_EMAIL_RECIPIENTS` GitHub secret. Also found and fixed: `Database:Provider` was unset in `appsettings.Production.json`/`Staging.json`, meaning a real deploy would have silently fallen through to the untested Postgres code path instead of SQL Server; and neither deploy workflow ever configured the deployed App Service's own runtime Application Settings (DB connection, JWT key, SMTP, CORS) after zip-deploying code — added `az webapp config appsettings set` steps to both. Verified end-to-end via a real live local run (not just code review): confirmation email, split team emails, and duplicate-hold handling all fire correctly (added 2026-09-10) |
| R-30 | Every case-activity-timeline entry and note across the whole app showed the acting staff member's raw GUID instead of their name (`ActorName`/`AuthorName` were literally set to `ctx.UserId`, 11 call sites) — a core dashboard feature (the case audit trail) silently broken since `IChapterContextService` never exposed a display name to begin with | Low | Medium | **Fixed 2026-09-10**, found via a live smoke test walking the actual running app end-to-end rather than code review alone: added `IChapterContextService.UserName` (from the JWT's given/surname claims, no extra DB call needed) and swapped all 11 sites from `ctx.UserId` to `ctx.UserName`. Also fixed a related gap found the same way: `POST /requests/{id}/notes` with missing content threw an unhandled 500 (raw SQLite `NOT NULL` constraint exception) instead of a clean 400 — added input validation. Both fixes verified live afterward against a fresh build; 415/415 tests still pass (added 2026-09-10) |
| R-31 | Duda-embeddable intake form (R-26) and the whole platform remain undeployed — no Azure App Service exists yet, only the deploy pipeline code does | High | High | **Open, needs Azure account access not available to the assistant** (same underlying blocker as R-20's open action, now also gating R-26's form and R-29's email fix from ever running for real). `API_BASE_URL` in `docs/duda-embed/prayer-care-intake.html` is still a placeholder; `AllowedOrigins`/`vars.DUDA_SITE_ORIGIN` need the real Duda domain once known; `JWT_SIGNING_KEY` and `DB_CONNECTION_STRING` secrets don't exist yet; SMTP secrets exist but have never been verified as live/working credentials. Fastest path identified: `az webapp up` (creates the App Service and deploys in one command, free `F1` tier) rather than the full portal-provisioning + CI-secrets flow, as a way to get something publicly testable before the complete production pipeline is wired up (added 2026-09-10) |
| R-32 | **Critical, found 2026-09-24:** `POST /api/v1/auth/register` was anonymous and honored any role in the request, so anyone on the internet could have created their own HQAdmin account on production | High | High | **Fixed** (`3d91275`/`c3efec5`, live via PR #65): creating accounts is HQ-admin only; `Auth:AllowOpenRegistration` exists for the test suite only and is off elsewhere; regression tests run against a production-like host. **Open action:** review the production Users list for accounts nobody recognises, since the hole was open until that deploy |
| R-33 | Dev seed data never reaches production (Development-only, separate SQLite vs SQL Server databases, fixed IDs and fake money would corrupt real data), leaving QA with nothing to test | Medium | Medium | Built a marked, undeliverable, removable **QA Sample Data** loader (39 families / 5 volunteers, `.invalid` emails, SAMPLE badges, no email to/about them, excluded from CRM export, no financial data). Verified on SQL Server LocalDB including the production upgrade path. **Open:** the production Load click fails for an unknown reason; failure reasons are now reported on the page (`545f46d`, needs deploy) |
| R-34 | Production could not send any email (no provider configured — everything was log-only), so families, volunteers and the team would never receive request emails | High | High | SocketLabs HTTP API wired in with SMTP fallback, Reply-To and plain-text bodies; provider banner and "Send test" on the Request Emails page. **Open, needs client/infra:** SocketLabs server id + API key, verified sender domain, Whitney's team addresses as GitHub secrets |
| R-35 | Production schema is created once by `EnsureCreated`, which never alters existing tables — every new column/table this session (mailing `Kind`, family grief answer, follow-up reminder flag, assignment rules) depends on idempotent startup bootstrap scripts that need ALTER/CREATE rights for the app's DB login | Medium | High | Four bootstraps written and verified on SQLite and on SQL Server LocalDB (old schema → upgrade → run twice → load/remove). Failures are logged and never crash startup. **Open:** confirm on production that the DB login can ALTER/CREATE; move to real migrations when prod DB access allows |
| R-36 | `stage` → `main` PRs showed "conflicting" although the content merged cleanly, blocking deploys (criss-cross merge commits from PR #58/#60) | Low | Medium | **Resolved 2026-09-24** by a history-only merge of `main` into `kremer-dev` (PR #63); flow is kremer-dev → stage → main via PR, never direct commits to protected branches |
| R-37 | "Group training" requested but never defined (training sessions? co-assigned volunteers? certifications?), risking a feature built to the wrong meaning | Medium | Low | **Open** — awaiting the requirement from the client before any build; nearest existing features are Events "Workshop" and volunteer certifications |
