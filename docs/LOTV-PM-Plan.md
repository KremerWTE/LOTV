...
| R-18 | JotForm intake webhook silently drops/corrupts data when the live form's question labels drift from the hardcoded `knownLabels` list in `Program.cs` | High | High | Confirmed actively happening: "Children for Bracelet" (required on every submission) was never in the list, so `Family.ChildrenInitials` has come through `null` on every real submission; this session's field cleanup on the live form introduced 2 more label mismatches; **open action**: reconcile `knownLabels` against the current live form and verify against a real test submission (added 2026-08-05) |
| R-19 | Third-party form-builder tool (JotForm) edits are not reliably atomic — an edit call can report success while silently corrupting unrelated settings | Medium | High | Discovered 2026-08-05: a targeted email-settings edit corrupted ~40 unrelated form-level properties in one call. Mitigation going forward: verify every JotForm edit via a read-only API re-fetch before trusting it; prefer manual builder-UI edits for email/notification settings until tool reliability is re-established; **open action**: confirm ministry has completed the manual revision-history rollback to 2026-07-22 11:59:27 (added 2026-08-05) |
