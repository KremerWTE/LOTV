...
| R-20 | `docker compose up` had never actually been run before 2026-08-13 despite the Dockerfiles/compose file being marked complete in Phase 6 — deployment path was unverified | Low | High | **Fixed 2026-08-13**: ran the full stack for the first time and found 4 real bugs — (1) `Events`/`VolunteerPending` Razor components sharing a class name across namespaces, fine under `dotnet build` but fatal (RZ9985) under `dotnet publish`/Docker; (2) API's non-root container couldn't write to its own unowned `/data` SQLite volume mount; (3) the healthcheck shelled out to `wget`, absent from the `aspnet:9.0` base image, so the container was permanently "unhealthy"; (4) nginx's `gzip_static` served a build-time-stale compressed `appsettings.json` sibling instead of the file rewritten at container start, so the Web container silently called the wrong API URL no matter what `API_BASE_URL` was set to. All 4 fixed and reverified live via Playwright (zero console errors, correct containerized API calls). CI now runs `dotnet publish` as a PR gate so bug (1)'s class can't recur silently. **Open action**: hosting platform (Azure/Railway/Fly.io/AWS/other) still not specified by the client despite confirming a decision was made — `deploy-staging.yml`/`deploy-production.yml`'s actual deploy step remains a placeholder until that's known (added 2026-08-13) |
| R-21 | Real intake (JotForm webhook or public `/apply`) never created a bereavement `FollowUpTracker` — only the one-time historical import ever populated that table, discovered by cross-checking webhook field coverage against the ministry's actual live-use spreadsheet rather than assuming the JotForm schema was sufficient ground truth | Low | High | **Fixed 2026-08-31**: added `CreateFollowUpTrackerIfLossKnownAsync`, called from both intake paths, gated on `Family.DateOfLoss` being set. Milestone offsets (+21 days, +3/+6/+11 calendar months) matched exactly against real due-date deltas in the ministry's live `SM (2026)` tracking sheet. Surfaced and fixed a second bug in the process: `GET /api/v1/follow-up-trackers` 500'd with an infinite `Tracker.Milestones` ↔ `Milestone.FollowUpTracker` reference cycle the instant a tracker was created with both sides tracked in the same EF context (the historical import apparently never triggered EF's relationship fixup the same way) — fixed with a scoped `[JsonIgnore]` on the unused back-reference rather than a global `ReferenceHandler` change. 4 new tests, verified live via Playwright. **Open, unfixed**: the same spreadsheet audit found "How did you hear about us?" is rich free text in real usage (actual referrer names/context) but the live JotForm form only offers a fixed dropdown — real referral detail will be lost going forward; needs either an "Other, specify" field added to the live form or an explicit sign-off that the trade-off is acceptable (added 2026-08-31) |
